...
var _ = Describe("[vendor:cnv-qe@redhat.com][level:component]DataVolume tests", func() {
	...
	Describe("DataVolume - WitForFirstConsumer", func() {
		...
		BeforeEach(func() {
			previousValue, err := utils.DisableFeatureGate(f.CdiClient, controller.SkipWaitForFirstConsumerVolumes)
			Expect(err).ToNot(HaveOccurred())
		...
		AfterEach(func() {
			if original != nil && *original {
				// restore
				_, err := utils.EnableFeatureGate(f.CdiClient, controller.SkipWaitForFirstConsumerVolumes)
				Expect(err).ToNot(HaveOccurred())
			}
		})
