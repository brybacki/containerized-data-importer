...
func EnableFeatureGate(client *cdiclientset.Clientset, feature string) (*bool, error) {
	...
	config, err := client.CdiV1alpha1().CDIConfigs().Get(common.ConfigName, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
	...
	config, err = client.CdiV1alpha1().CDIConfigs().Update(config)
	if err != nil {
		return nil, err
	}
...
func HasFeature(config *v1alpha1.CDIConfig, featureGate string) bool {
